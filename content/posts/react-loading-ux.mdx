...
`gcTime`이 `staleTime`보다 상위 개념입니다. 캐시가 메모리에 남아있는 한 로딩은 발생하지 않습니다. `staleTime`은 그 캐시로 백그라운드 리패치를 할지 말지를 결정할 뿐입니다.
...
백그라운드 리패치는 사용자에게 알릴 필요가 없습니다. opacity 처리나 스피너를 표시하는 순간 캐시 전략의 UX적 이점이 사라집니다.
