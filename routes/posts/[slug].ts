...
export default defineEventHandler(async (event) => {
  ...
  const body = `<div id=ct><a href="/all" class=cb>← 모든 글</a>${
    tags.length
      ...
      : `<div class=cm></div>`
  }<h1 class=ct>${post.title}</h1><div class=am><b>${fmtDate(post.date)}</b><span>·</span><span>${post.minutes} min read</span><span>·</span><span>Load 28</span></div>${aiTag}<div class=dv></div><div class=bd>${articleBlocks(post.html)}</div><div class=ae><a href="/all" class=cb>← 모든 글 보기</a></div></div>`;
