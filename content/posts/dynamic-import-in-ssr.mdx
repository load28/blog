...
| Suspense 필요 | ✅ 필수 | ❌ 선택적 (loading 옵션) |
| SSR 제어 | ❌ 불가능 | ✅ 가능 (ssr 옵션) |
| 표준 API | ✅ React 표준 | ❌ Next.js 전용 |
| 로딩 UI | Suspense fallback | loading 옵션 또는 Suspense |
| 코드 스플리팅 | ✅ 지원 | ✅ 지원 |
| 번들러 독립성 | ✅ 독립적 | ⚠️ Next.js 의존 |
...
```tsx
// ❌ 서버에서 에러 발생 (window is not defined)
const MapComponent = () => {
  const map = new window.google.maps.Map(...);
  return <div ref={mapRef} />;
};

// ✅ ssr: false로 클라이언트 전용 렌더링
