...
# 다른 브랜치의 세션 이어받기 (rewind와 별개 명령어)
entire resume feature/auth-branch
```

## Rewind와 Resume: 별개의 명령어

`entire rewind`와 `entire resume`는 별개의 독립 명령어입니다.

### Rewind — 현재 세션 내에서 되감기

리윈드는 현재 세션에서 이전 체크포인트로 **코드 파일**과 **세션 로그(JSONL 파일)**를 롤백합니다. Claude의 컨텍스트 윈도우는 Entire가 직접 제어할 수 없으므로, Claude Code를 종료 후 재시작해서 컨텍스트를 초기화해야 합니다.
...
### Resume — 다른 브랜치의 세션 이어받기

`entire resume`는 리윈드의 후속 단계가 아니라, **브랜치를 전환하고 해당 브랜치의 최신 체크포인트 세션을 복원**하는 별도 명령어입니다. 동료가 작업하던 브랜치를 이어받거나, 다른 브랜치에서 중단했던 작업을 재개할 때 사용합니다.
