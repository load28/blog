---
title: 'Yarn'
description: 'Exploring the concept of Yarn and its features'
...
- `.yarn/cache`와 `.pnp.cjs`를 Git에 커밋하면, 브랜치를 변경할 때 Git이 해당 브랜치의 의존성 캐시를 자동으로 체크아웃하므로 별도로 yarn install을 실행할 필요가 없어진다 (zero install이라 부른다)
