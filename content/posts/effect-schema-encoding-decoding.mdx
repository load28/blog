...
이것이 핵심적인 차이입니다. Zod의 `transform`은 **파싱 파이프라인의 한 단계**이고, Effect Schema의 인코딩/디코딩은 **양방향 변환 계약**입니다.

| | Effect Schema | Zod |
|:--|:--|:--|
| 디코딩 (외부 → 내부) | `Schema.decodeUnknownSync` | `z.parse` |
| 인코딩 (내부 → 외부) | `Schema.encodeSync` | 지원하지 않음 |
| 타입 추출 | `Type`, `Encoded` 별도 추출 | `z.infer`(출력 타입만) |
| 양방향 변환 | 기본 설계 | `transform`은 단방향 |
...
HTML form의 checkbox는 `"on"` 또는 `"off"` 문자열을 보냅니다. 이걸 `boolean`으로 변환하는 스키마를 만들어봅시다.
