...
`Endpoint<TParams, TResponse>` 대신 `Endpoint<any, any>`로 제약하는 이유는 **공변성(covariance)** 문제 때문입니다.
...
`UserParams`(`{ userId: string }`)는 명시적 속성을 가진 타입이고, `Record<string, string>`은 인덱스 시그니처 타입입니다. TypeScript에서 이 둘은 호환되지 않을 수 있습니다. `any`를 쓰면 이 제약을 우회하고, 실제 타입 체크는 `ExtractParams`와 `ExtractResponse`에서 수행합니다.

제약 조건의 역할 분리라고 볼 수 있습니다. **`extends AnyEndpoint`은 "이것은 Endpoint 구조를 가진 값이다"라는 구조 검증**만 담당하고, **`ExtractParams<TEndpoint>`이 실제 타입 정확성을 보장**합니다.
